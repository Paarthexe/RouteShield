  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (data.routes && data.routes.length === 1) {
        setNotice(
          "Only one feasible corridor was discovered for this terrain.",
        );
      }
    } catch (err) {
      ...
      <main className="flex-1 w-full max-w-[1720px] mx-auto p-4 sm:p-5 grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="flex items-start gap-2 rounded-xl border border-slate-800 bg-slate-900/50 px-4 py-3 text-[11px] leading-relaxed text-slate-500">
            ...
            <span>
              Decision-support prototype: route conditions can change during an
              active emergency. Current results are based on the configured
              routing and physical-world data sources and are not an official
